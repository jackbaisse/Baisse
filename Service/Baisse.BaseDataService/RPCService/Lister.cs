...
using Baisse.BaseDataCommon.RPC.RpcServer;

namespace Baisse.BaseDataService.RpcServer
{
    public class Lister<T> where T : new()
    {
        private Server m_socket;
        private T _methodName;
        /// <summary>
        /// 最大连接数
        /// </summary>
        ...
        void m_socket_ReceiveClientData(AsyncUserToken token, byte[] buff)
        {
            ...
            ///发送消息
            byte[] message = Encoding.UTF8.GetBytes(result);
        ...
        public string GetMethodInvoke(string msg)
        {
            try
            {
                if (string.IsNullOrEmpty(msg) || msg == "null") return msg;
            ...
            {
                var response = ResponseContent.Fail<string>("调用目标方法异常：" + e.Message);
