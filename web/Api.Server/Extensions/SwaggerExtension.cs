        ...
        public static void UseSwaggerMiddleware(this IApplicationBuilder app, IApiVersionDescriptionProvider provider)
        {
            ...
            app.UseSwaggerUI(option =>
            {
                foreach (var item in provider.ApiVersionDescriptions)
                {
                    ...
                    option.SwaggerEndpoint($"/swagger/{item.GroupName}/swagger.json", "Baisse.Blog.WebApi API" + item.ApiVersion);
                }
                //option.RoutePrefix = string.Empty;
                option.DocumentTitle = "Baisse.Blog API";
            ...
            services.AddSwaggerGen(options =>
            {
                ...
                foreach (var item in provider.ApiVersionDescriptions)
                {
                    ...
                    options.SwaggerDoc(item.GroupName, new OpenApiInfo
                    {
                        ...
                        Title = $"Baisse.Blog API {item.ApiVersion}",
                        Description = "API for Baisse.Blog",
                        Contact = new OpenApiContact() { Name = "Baisse", Email = "1297087111@qq.com", Url = new Uri("https://hdefu.com") }
                    });
                }

                // 设置XML文档
                options.IncludeXmlComments(Path.Combine(AppContext.BaseDirectory, $"{typeof(Startup).Assembly.GetName().Name}.xml"), true);
                //options.IncludeXmlComments(Path.Combine(AppContext.BaseDirectory, $"{typeof().Assembly.GetName().Name}.xml"), true);
