...
namespace WindowsFormsKeyboardHook
{
    public class KeyboardHook
    {
        ...
        Win32Api.HookProcKey KeyboardHookDelegate;
        ...
        public void SetHook()
        {
            ...
            KeyboardHookDelegate = new Win32Api.HookProcKey(KeyboardHookProc);

            Process cProcess = Process.GetCurrentProcess();//获取当前进程

            ProcessModule cModule = cProcess.MainModule; //获取进程主模块

            var mh = Win32Api.GetModuleHandle(cModule.ModuleName);//获取句柄

            hHook = Win32Api.SetWindowsHookEx(Win32Api.WH_KEYBOARD_LL, KeyboardHookDelegate, mh, 0);//安装勾子
